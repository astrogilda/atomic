...
**Learning M16 — pre-write failure is safe; mid-write failure is not yet a green claim.** `ATOMIC_TEST_BRIDGE_FAIL_BEFORE_MATERIALIZE=1` proves a failure after planning but before `Repository::switch_view` mutates nothing and retries cleanly. Native switch now validates a complete render-only target before publishing the target pointer, shelving files, or removing tracked paths, so graph/preload/content failures are non-destructive. It still publishes the pointer before non-transactional filesystem execution and has no rollback for a later write/removal failure. The harness deliberately skips mid-write rollback. Phase 1 operations/effect receipts plus a reusable deterministic materialization plan are acceptance blockers before bridge switching can be automatic.
