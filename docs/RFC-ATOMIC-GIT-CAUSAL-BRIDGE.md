...
Any failure in snapshot, split, import, provenance finalization, or attestation writes `SessionStatus::Incomplete { reason, unbound_commits, unrecorded_paths }` durably and returns non-zero to the hook. Turn-end never reports success with uncovered work. Recovery: `atomic agent repair <session>`.
...
| `pre-commit` | If a managed agent session is active and the staged tree is not covered by a snapshot → run snapshot first (so split-bind is possible) | Fails closed only for agent sessions |
...
2. Should an operation that cannot be reassembled into exact baseline→index and index→worktree patches cause Git commit capture to fail closed, or permit the commit but mark the managed session synthesized/incomplete for review?
3. Should `GitResolution` changes be replayable as Atomic conflict resolutions when the same conflict recurs across views?
...
- `tests/harness/39_git_bridge_mvp.sh`: offline Git-first reconciliation, explicit verify, status cleanliness, idempotency, complete path presence, zero-byte file, and non-destructive dirty refusal.
- `atomic-repository/tests/effective_history_test.rs`: nested inherited history/content closure and tracked-empty `Some(Vec::new())`.

**Results (2026-09-02):** bridge direction/identity suite 19 passed; collision-planner suite 15 passed; focused repository regressions 4 passed; bidirectional/materialization/raw-Git harness 143 passed; materialization-safety harness 81 passed with 1 explicit journal skip. Standalone expected-red mid-write harness: 12 desired assertions pass, 7 fail as intended.
...
**Learning M2 — presence cannot be inferred from bytes.** `get_file_content*` converted an empty result to `None` after already proving TREE/INODES presence. Returning `Some(Vec::new())` fixed the focused regression and demonstrates why Phase N requires typed presence throughout parallel materialization as well; the MVP does not yet fix the parallel writer's empty-file path.

**Learning M3 — existing view APIs are enough to falsify the clean-path theory.** `visible_file_paths` plus `get_file_content_on_view` provide a useful complete-tree oracle for regular files. They are not the final manifest engine: current path discovery and content retrieval still differ in dependency expansion (N13), and modes/types/filters are unsupported.
...
**Learning M13 — symbolic branch identity is part of the checkpoint.** `git checkout -b topic` may change only HEAD's symref while commit/tree OIDs remain identical. OID-only classification incorrectly says no-op. The MVP now compares checkpoint view/ref identity as well as OID and Atomic state. At the unchanged bound commit it creates a self-contained Shared Atomic view populated from the checkpoint view's effective history, aligns metadata, and reindexes without materialization; hashes, mtimes, and inode numbers remain unchanged.
...
**Learning M16 — pre-write failure is safe; mid-write failure is not yet a green claim.** `ATOMIC_TEST_BRIDGE_FAIL_BEFORE_MATERIALIZE=1` proves a failure after planning but before `Repository::switch_view` mutates nothing and retries cleanly. Current native switch publishes the target pointer before non-transactional removals/parallel writes and has no working-copy rollback; per-file errors may be swallowed. The harness deliberately skips mid-write rollback. Phase 1 operations/effect receipts plus a deterministic materialization plan are acceptance blockers before bridge switching can be automatic.

**Learning M17 — deterministic first-mutation interruption reproduces the corruption class.** Debug-only `ATOMIC_FAIL_SWITCH_AFTER_FIRST_TRACKED_REMOVAL` sorts source-only paths and fails immediately after the first successful removal. `red_switch_fail_after_first_tracked_removal.sh` is deliberately excluded from normal `[0-9][0-9]_*.sh` discovery and asserts the future contract. Today it exits red: Git remains on source; Atomic pointer has advanced to target; checkpoint remains source; the first source file is absent while the second remains; normal retry refuses checkpoint mismatch; Git and Atomic status disagree. This exact 12-pass/7-fail signature is the minimum acceptance target for the operation/effect journal.

**Validation caveat M18 — native delete propagation has an independent red regression.** The broad `cargo test -p atomic-repository repository::tests` run currently reports `whole_file_delete_insert_removes_file_on_target_view` failing: inserting a whole-file deletion then materializing leaves old disk content. This is consistent with Phase N's typed presence/deletion work and is not caused by the debug failpoint, but it blocks claiming broad repository-suite health.
...
5. Tracked zero-byte content remains present.
