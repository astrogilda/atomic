...
**Results (2026-09-02):** bridge direction/identity suite 19 passed; collision-planner suite 15 passed; focused repository materialization/deletion regressions 8 passed; effective-history/visibility regressions 6 passed; `atomic-core` and `atomic-repository` suites pass; bidirectional/materialization/raw-Git harness 146 passed; materialization-safety harness 81 passed with 1 explicit journal skip. Standalone expected-red mid-write harness remains the Phase 1 recovery gate.
...
**Learning M16 — pre-write failure is safe; mid-write failure is not yet a green claim.** `ATOMIC_TEST_BRIDGE_FAIL_BEFORE_MATERIALIZE=1` proves a failure after planning but before `Repository::switch_view` mutates nothing and retries cleanly. Current native switch publishes the target pointer before non-transactional removals/parallel writes and has no working-copy rollback; materialization errors now propagate but cannot undo prior filesystem effects. The harness deliberately skips mid-write rollback. Phase 1 operations/effect receipts plus a deterministic materialization plan are acceptance blockers before bridge switching can be automatic.
